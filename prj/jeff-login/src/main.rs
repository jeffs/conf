...
use std::{env, error::Error, ffi, fs, io, path::Path};
...
fn main() {
    ...
    write_json(&platform.env, &path, &json).unwrap_or_else(|e| panic!("{}: {e}", json.display()));

    // Save exports for POSIX shells.
    let sh = var.join("env.sh");
    write_sh(&sh, &platform.env, &path).unwrap_or_else(|e| panic!("{}: {e}", sh.display()));
}
