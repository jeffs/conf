def fc-list [] {
  ...
  let family = $family_style | default -e '' family | select family | update family {split row ',' | filter {is-not-empty}}
  let style = $family_style | default -e '' style | select style | update style {split row ',' | filter {is-not-empty}}
