...
> - **`prj/mkenv` generating `env.sh` + `env.json` for POSIX vs Nu/Xonsh → two `.tmpl` files over shared data.** This is the textbook chezmoi use case: one source of truth, multiple rendered shells. You'd lose the typed Rust schema but gain the diff/apply/dry-run lifecycle.
> - **Editor unification (helix/nvim/vscode/ideavim) → same templating story.** Probably the strongest reason to adopt it, since you don't have an analogue of `prj/mkenv` for editors yet and you'd otherwise be building one.
